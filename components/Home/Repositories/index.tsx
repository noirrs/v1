...
export const Repositories: FC = () => {
  ...
  useEffect(() => {
    fetch("https://api.github.com/users/Noirrs/repos", {})
    ...
    <div className="" id="repos">
      <div className="lg:mt-60 mt-40">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 lg:ml-14 mt-4 gap-y-4">
          {repositories &&
            repositories
              .filter((a: any) => !a.forked)
              .map((repo: any, index: Number) => {
                return (
                  <Tilt
                    className="w-96 rounded-lg h-28 mx-auto  over:scale-110 bg-[#272930]"
                    options={{ max: 20 }}
                    key={index}
                  >
                    <Link href={repo.url}>
                      <a href={repo.url} target="_blank" rel="noreferrer">
                        <div className="">
                          ...
                          {repo.language && (
                            <div className="flex">
                              <div
                                className="w-3 h-3 rounded-full mt-7 ml-2"
                                style={{
                                  backgroundColor: Colors[repo.language].color,
                                }}
                              ></div>
                              <p className="text-white ml-1.5 mt-[22px]">
                                {repo.language}
                              </p>
                            </div>
                          )}
